...
class Coupling(Flow):
    ...
    def forward(self, inputs, context=None):
        ...
        # inputs = torch.stack([inputs[:, 1], inputs[:, 0]], dim=-1).to(self.identity_features.device)
        # inputs = (inputs[1], inputs[0], *inputs[2:])
        # split = int(self.features / 2)
        # inputs = torch.cat([inputs[:, split:], inputs[:, :split]], dim=1)
        ...
        split = int(self.features / 2)
        outputs = torch.cat([outputs[:, split:], outputs[:, :split]], dim=1)
    ...
    def inverse(self, inputs, context=None):
        ...
        # inputs = torch.stack([inputs[:, 1], inputs[:, 0]], dim=-1)
        split =int(self.features/2)
        inputs = torch.cat([inputs[:, split:], inputs[:, :split]], dim=1)
        identity_split = inputs[:, self.identity_features, ...]
        transform_split = inputs[:, self.transform_features, ...]
