...
class PeriodicFeaturesElementwise(nn.Module):
    ...
    def forward(self, inputs):
        ...
        # k = torch.tensor(torch.pi / 2.5, device=inputs.device)  # Fixed scale factor
        cos_part = torch.cos(self.scale * inputs_)
        sin_part = torch.sin(self.scale * inputs_)
        inputs_ = torch.cat([cos_part, sin_part], dim=-1)
        ...
        # out = inputs_.view(inputs_.shape[0], -1, 2)
